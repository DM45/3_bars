...
def validate_data(data, longitude, latitude):
    validate_error = 0
    if data is None:
        validate_error = 1
...
def get_seats_count_and_name(data):
    seats_and_name = [
        (datas.get("SeatsCount"), datas.get("Name"))for datas in data]
...
def get_closest_bar(data, longitude, latitude):
    coord_diff_and_name = [
        ...
        datas.get("Name")) for datas in data
    ]
